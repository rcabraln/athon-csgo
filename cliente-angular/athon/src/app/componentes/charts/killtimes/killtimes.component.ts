...
import { KilltimesService} from 'src/app/services//charts/killtimes.service';
...
@Component({
  selector: 'app-killtimes',
  templateUrl: './killtimes.component.html',
  styleUrls: ['./killtimes.component.css']
})

export class KilltimesComponent implements OnInit{
  private chart: am4charts.XYChart;
  data: any
  chartsdata = []

  constructor(@Inject(PLATFORM_ID) private platformId, private zone: NgZone, private KilltimesService: KilltimesService) {   }
  async ngOnInit() {this.getData()}

  
  getData(){
    // buscando os dados do serviço e trasformando em array de dicionários  
    this.KilltimesService.getChart().subscribe(data => {
      this.data = data;
      ...
      this.zone.runOutsideAngular(() => {      
        this.browserOnly(() => {
          ...
          let chart = am4core.create("chartdiv-killtimes", am4charts.XYChart);
